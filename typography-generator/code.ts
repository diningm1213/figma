...
function rbgToHex(r: number, g: number, b: number) {
  console.log(r, g, b);
...
figma.codegen.on("generate", (event) => {
  ...
  if (event.node.type === "TEXT") {
    ...
    if (fill.type === "SOLID") {
      code = `<Typography type="${fontName.family.toLowerCase()}_${String(
        node.fontSize
      )}" weight="${String(node.fontWeight)}" color="${rbgToHex(
        ~~(fill.color.r * 255),
        ~~(fill.color.g * 255),
        ~~(fill.color.b * 255)
      ).toUpperCase()}">${node.characters}</Typography>`;
    }
  }
  // const code = `{
